...
import { db } from "./db";
import { eq, and, desc, sum, count, sql, gt } from "drizzle-orm";
...
export interface IStorage {
  ...
  createLoginVerificationCode(
    code: InsertLoginVerificationCode
  ...
  updateRecipientPersonalizedEmail(
    id: number,
  ...
  // Campaign emails
  createCampaignEmail(
    email: Omit<CampaignEmail, "id" | "createdAt">
  ...
  // Warm-up
  createWarmupEmail(
    email: Omit<WarmupEmail, "id" | "sentAt">
  ): Promise<WarmupEmail>;
  getWarmupStats(integrationId: number): Promise<{
    sent: number;
    received: number;
    opened: number;
    replied: number;
  ...
  findWarmupEmailByTrackingId(
    trackingId: string
  ): Promise<WarmupEmail | undefined>;
  updateWarmupEmailTracking(
    trackingId: string,
    field: "opened" | "replied" | "spam",
    value?: any
  ): Promise<WarmupEmail | undefined>;
  markWarmupEmailAsOpened(trackingId: string): Promise<WarmupEmail | undefined>;
  markWarmupEmailAsReplied(
    trackingId: string,
    replyBody?: string
  ): Promise<WarmupEmail | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateTrustedIpLastUsed(
    ...
    await db
      ...
      .where(
        and(eq(trustedIps.userId, userId), eq(trustedIps.ipAddress, ipAddress))
      );
  }
  ...
  async getUserTrustedIps(userId: string): Promise<TrustedIp[]> {
    return await db
      .select()
      .from(trustedIps)
      .where(and(eq(trustedIps.userId, userId), eq(trustedIps.isActive, true)))
    ...
    const [verificationCode] = await db
      ...
      .where(
        and(
          ...
          // sql`expires_at > NOW()`
          gt(loginVerificationCodes.expiresAt, new Date())
        )
      );
    return verificationCode;
  }
  ...
  async deleteExpiredLoginVerificationCodes(): Promise<void> {
    await db.delete(loginVerificationCodes).where(sql`expires_at < NOW()`);
  }
  ...
  async getAllConfig(): Promise<AdminConfig[]> {
    return await db.select().from(adminConfig).orderBy(adminConfig.configKey);
  }

  async deleteConfig(key: string): Promise<void> {
    await db.delete(adminConfig).where(eq(adminConfig.configKey, key));
  }
  ...
  async updateEmailIntegrationVerification(
    id: number,
    isVerified: boolean
  ): Promise<EmailIntegration> {
  ...
  async updateRecipientDeliverability(
    id: number,
    status: string
  ): Promise<Recipient> {
    const [recipient] = await db
  ...
  async updateRecipientPersonalizedEmail(
    id: number,
    personalizedEmail: string
  ): Promise<Recipient> {
    console.log(
      `Saving personalized email for recipient ${id}, length: ${personalizedEmail.length}`
    );
    const [recipient] = await db
      .update(recipients)
    ...
    if (recipient?.personalizedEmail) {
      console.log(
        `Verified email saved successfully, actual length: ${recipient.personalizedEmail.length}`
      );
    } else {
  ...
  async removeInvalidRecipients(listId: number): Promise<number> {
    try {
      const result = await db
        ...
        .where(
          and(
            ...
            eq(recipients.deliverabilityStatus, "invalid")
          )
        );
      ...
      try {
        ...
        console.error(
          "Fallback removeInvalidRecipients failed:",
          fallbackError
        );
        throw fallbackError;
      }
  ...
  async getCleanRecipients(listId: number): Promise<Recipient[]> {
    return await db
      ...
      .where(
        and(
          eq(recipients.listId, listId),
          eq(recipients.deliverabilityStatus, "valid")
        )
      );
  }
    ...
    return {
      ...
      openRate:
        campaign.sentCount > 0
          ? (((campaign.openedCount || 0) / campaign.sentCount) * 100).toFixed(
              1
            )
          : "0",
      clickRate:
        campaign.sentCount > 0
          ? (((campaign.clickedCount || 0) / campaign.sentCount) * 100).toFixed(
              1
            )
          : "0",
      deliveryRate:
        campaign.sentCount > 0
          ? (
              ((campaign.deliveredCount || 0) / campaign.sentCount) *
              100
            ).toFixed(1)
          : "0",
    };
  ...
  async updateCampaignEmailStatus(
    id: number,
    status: string,
    timestamp?: Date
  ): Promise<CampaignEmail> {
  ...
  async getWarmupStats(integrationId: number): Promise<{
    sent: number;
    received: number;
    opened: number;
    replied: number;
  ...
  async getTodayWarmupStats(
    integrationId: number
  ): Promise<WarmupStats | undefined> {
  ...
  async findWarmupEmailByTrackingId(
    trackingId: string
  ): Promise<WarmupEmail | undefined> {
    const [warmupEmail] = await db
      .select()
  ...
  async updateWarmupEmailTracking(
    trackingId: string,
    field: "opened" | "replied" | "spam",
    value?: any
  ): Promise<WarmupEmail | undefined> {
  ...
  async markWarmupEmailAsReplied(
    trackingId: string,
    replyBody?: string
  ): Promise<WarmupEmail | undefined> {
  ...
  async findWarmupEmailBySubject(
    integrationId: number,
    subject: string
  ): Promise<WarmupEmail | undefined> {
    const [email] = await db
  ...
  async getCampaignEmailByTracking(
    trackingPixelId: string
  ): Promise<CampaignEmail | undefined> {
    const [campaignEmail] = await db
      .select()
      .from(campaignEmails)
  ...
  async markCampaignEmailAsOpened(trackingPixelId: string): Promise<void> {
    const campaignEmail = await this.getCampaignEmailByTracking(
      trackingPixelId
    );
    if (campaignEmail && !campaignEmail.openedAt) {
      ...
      if (campaignId) {
        ...
        if (campaign) {
          ...
          await this.updateCampaignStats(parseInt(campaignId), {
  ...
  async getDashboardStats(userId: string): Promise<{
    ...
    const [activeCampaignStats] = await db
      ...
      .from(campaigns)
      .where(and(eq(campaigns.userId, userId), sql`status != 'deleted'`));
