...
async function sendAuthEmail(
  type: "verification" | "reset" | "login_verification",
  config: any
): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    const child = spawn(pythonCmd, ["server/email_auth.py", type, configJson]);
    let output = "";
...
async function startCampaignSending(campaignId: number, userId: string) {
  try {
    ...
    const recipients = await storage.getListRecipients(
      campaign.recipientListId
    );
    ...
    const limits =
      planLimits[user?.plan as keyof typeof planLimits] || planLimits.demo;
...
async function sendCampaignEmails(
  ...
  const sentRecipientIds = new Set(
    sentEmails.map((email) => email.recipientId)
  );

  // Filter out recipients who have already been sent emails
  const remainingRecipients = recipients.filter(
    (recipient) => !sentRecipientIds.has(recipient.id)
  );

  console.log(
    `Campaign ${campaignId}: Found ${sentRecipientIds.size} already sent, ${remainingRecipients.length} remaining to send`
  );
  ...
  for (let i = 0; i < remainingRecipients.length; i++) {
    ...
    try {
      ...
      let integration;
      console.log(
        `ROTATION DEBUG - Campaign ${campaignId}: emailRotationEnabled=${
          campaign.emailRotationEnabled
        }, emailRotationIds=${JSON.stringify(
          campaign.emailRotationIds
        )}, length=${campaign.emailRotationIds?.length}`
      );

      if (
        campaign.emailRotationEnabled &&
        campaign.emailRotationIds &&
        campaign.emailRotationIds.length > 0
      ) {
        ...
        const accountIndex =
          Math.floor(currentSendingIndex / emailsPerAccount) %
          campaign.emailRotationIds.length;
        ...
        console.log(
          `ROTATION ACTIVE - currentSendingIndex=${currentSendingIndex}, emailsPerAccount=${emailsPerAccount}, accountIndex=${accountIndex}, rotationIntegrationId=${rotationIntegrationId}, using account ${
            accountIndex + 1
          }/${campaign.emailRotationIds.length} (${integration?.email})`
        );
      } else {
        // Use single email account
        integration = await storage.getEmailIntegration(
          campaign.emailIntegrationId
        );
        console.log(
          `NO ROTATION - using primary account ID ${campaign.emailIntegrationId}: ${integration?.email}`
        );
      }
      ...
      const hasPersonalizeEmailField = campaign.body.includes(
        "{personalize_email}"
      );
      const hasTraditionalFields =
        campaign.body.includes("{name}") ||
        campaign.body.includes("{lastName}") ||
        campaign.body.includes("{position}") ||
        campaign.body.includes("{company}");
      ...
      if (hasPersonalizeEmailField) {
        ...
        if (recipient.personalizedEmail && recipient.personalizedEmail.trim()) {
          ...
          emailBody = campaign.body.replace(
            /{personalize_email}/g,
          ...
          console.log(
            `Using fallback content with traditional fields for ${recipient.email}`
          );
        } else {
          // No personalized email and fallback disabled - skip recipient
          console.log(
            `Skipping recipient ${recipient.email} - no personalized content and fallback disabled`
          );
          continue;
        }
      } else {
        ...
        console.log(
          `Using traditional field replacement for ${recipient.email}`
        );
      }
      ...
      console.log(
        `Personalizing email for ${recipient.name} ${recipient.lastName} (${
          recipient.email
        }) from ${recipient.companyName || "Unknown Company"}`
      );
      ...
      console.error(
        `Error sending email to ${remainingRecipients[i]?.email}:`,
        error
      );
    }
  }
  ...
  if (remainingRecipients.length > 0 && sentToday > 0) {
    ...
    if (
      updatedCampaign &&
      updatedCampaign.sentCount >= updatedCampaign.totalRecipients
    ) {
...
async function processFollowUps() {
  try {
    ...
    const followUpCampaigns = campaigns.filter(
      (campaign) => campaign.followUpEnabled && campaign.status === "completed"
    );
    ...
    for (const campaign of followUpCampaigns) {
      ...
      for (const email of campaignEmails) {
        ...
        const timeDiffDays =
          (now.getTime() - sentTime.getTime()) / (1000 * 60 * 60 * 24);

        console.log(
          `Email ${email.id}: sent ${timeDiffDays.toFixed(
            1
          )} days ago, needs ${followUpDays} days delay`
        );

        // If enough time has passed and conditions are met
        if (timeDiffDays >= followUpDays) {
          const shouldSendFollowUp = await checkFollowUpConditions(
            email,
            campaign
          );
...
async function checkFollowUpConditions(
  email: any,
  campaign: any
): Promise<boolean> {
...
async function sendFollowUpEmail(originalEmail: any, campaign: any) {
  try {
    ...
    if (
      ...
      const followUpCount = allFollowUpEmails.filter(
        (email) => email.followUpId
      ).length;

      // Calculate which email account to use for follow-up rotation
      const emailsPerAccount = campaign.emailsPerAccount || 1;
      const accountIndex =
        Math.floor(followUpCount / emailsPerAccount) %
        campaign.emailRotationIds.length;
      const rotationIntegrationId = campaign.emailRotationIds[accountIndex];
      integration = await storage.getEmailIntegration(rotationIntegrationId);

      console.log(
        `FOLLOW-UP ROTATION - followUpCount=${followUpCount}, accountIndex=${accountIndex}, using account ${
          accountIndex + 1
        }/${campaign.emailRotationIds.length} (${integration?.email})`
      );
    } else {
      ...
      console.log(
        `FOLLOW-UP NO ROTATION - using primary account: ${integration?.email}`
      );
    }
    ...
    const trackingPixelId = `${campaign.id}_${
      recipient.id
    }_followup_${Date.now()}`;
...
async function checkPlanLimits(
  ...
  switch (resource) {
    ...
    case "recipients":
      return (
        (currentUsage.recipientsUploaded || 0) + amount <= limits.recipients
      );
    case "deliverability":
      return (
        (currentUsage.deliverabilityChecks || 0) + amount <=
        limits.deliverabilityChecks
      );
    case "personalization":
      return (
        (currentUsage.personalizedEmails || 0) + amount <=
        limits.personalizedEmails
      );
    case "warmup":
...
async function sendVerificationEmail(config: {
  ...
  return new Promise<void>((resolve, reject) => {
    const pythonCmd = process.platform === "win32" ? "py" : "python3"; // Use py on Windows, python3 on others

    const pythonScript = spawn(
      pythonCmd,
      ["server/email_auth.py", "verification", JSON.stringify(pythonConfig)],
    ...
    pythonScript.on("close", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(`Verification email sending failed with code ${code}`)
        );
      }
    });
  });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      try {
        ...
        if (isSmtpConfigured()) {
          console.log(
            "SMTP is properly configured, sending verification email..."
          );
          ...
          res.json({
            message:
              "Account created successfully. You can now log in. (Email verification disabled - no SMTP configured)",
            userId: user.id,
          });
        }
      } catch (emailError) {
        ...
        res.json({
          message:
            "Account created successfully. You can now log in. (Email verification failed)",
          userId: user.id,
        });
      }
    } catch (error) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (!isTrustedIp) {
        ...
        if (verificationCode) {
          const validCode = await storage.getLoginVerificationCode(
            verificationCode,
            user.id
          );
          ...
          try {
            if (isSmtpConfigured()) {
              ...
              return res.status(200).json({
                ...
                message:
                  "New device detected. Please check your email for a verification code to complete login.",
              });
            } else {
  ...
  app.post("/api/admin/test-email", adminAuth, async (req, res) => {
    try {
      ...
      // Get environment SMTP configuration
      ...
      if (!smtpHost || !smtpUsername || !smtpPassword || !smtpFromEmail) {
        return res.status(400).json({
          message: "SMTP configuration incomplete in environment variables",
        });
      }

      // Test verification email
      const testVerificationLink = `${req.protocol}://${req.get(
        "host"
      ...
      res
        .status(500)
        .json({ message: "Failed to send test email: " + error.message });
    }
  });
    ...
    try {
      ...
      if (!googleClientId || !googleClientSecret) {
        try {
          ...
          const clientSecretConfig = await storage.getConfig(
            "google_client_secret"
          );

          googleClientId = googleClientId || clientIdConfig?.configValue;
          googleClientSecret =
            googleClientSecret || clientSecretConfig?.configValue;
        } catch (error) {
          console.log(
            "Google OAuth config not found in database, checking env vars only"
          );
        }
      }
              ...
              try {
                ...
                if (!user) {
                  ...
                  user = await storage.createUser({
                    ...
                    googleId: profile.id, // store Google unique ID
                    emailVerified: true, // Google email is already verified
                    plan: "demo",
                    profileImageUrl: profile.photos?.[0]?.value,
                  });
                } else {
                  // Agar pehle user tha aur googleId null hai, to update karo
                  if (!user.googleId) {
  ...
  app.get("/api/auth/google", async (req, res, next) => {
    ...
    const hasGoogleClientId =
      ...
      (await storage.getConfig("google_client_id"))?.configValue;
    if (!hasGoogleClientId) {
      return res.status(501).json({
        message:
          "Google OAuth not configured. Please configure it in admin panel.",
      });
    }
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      try {
        ...
        const smtpFromName = process.env.SMTP_FROM_NAME || "Email SaaS";
        ...
        if (smtpHost && smtpUsername && smtpPassword && smtpFromEmail) {
          ...
          console.log(
            "❌ SMTP configuration incomplete - cannot send verification email"
          );
          // Auto-verify user since SMTP is not configured
          await storage.updateUserEmailVerified(user.id, true);
          res.json({
            message:
              "User created successfully. You can now log in. (Email verification disabled - no SMTP configured)",
            userId: user.id,
          });
        }
      } catch (emailError) {
        ...
        res.json({
          message:
            "User created successfully. You can now log in. (Email verification failed)",
          userId: user.id,
        });
      }
    } catch (error) {
  ...
  app.post("/api/auth/resend-verification", async (req, res) => {
    try {
      ...
      await sendVerificationEmail({
        ...
        verification_link: `${process.env.FRONTEND_URL}/verify?token=${verificationToken}`,
      });
  ...
  app.get("/api/dashboard/stats", isAuthenticated, async (req: any, res) => {
    try {
      const userId =
        req.user?.claims?.sub || req.user?.id || req.session?.manualUser?.id;
      if (!userId) {
        return res.status(401).json({ message: "User ID not found" });
      }
  ...
  app.get("/api/user/stats", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const serializedLimits = {
        ...userPlanLimits,
        emailsPerMonth:
          userPlanLimits.emailsPerMonth === Infinity
            ? "Infinity"
            : userPlanLimits.emailsPerMonth,
        recipients:
          userPlanLimits.recipients === Infinity
            ? "Infinity"
            : userPlanLimits.recipients,
        emailIntegrations:
          userPlanLimits.emailIntegrations === Infinity
            ? "Infinity"
            : userPlanLimits.emailIntegrations,
        deliverabilityChecks:
          userPlanLimits.deliverabilityChecks === Infinity
            ? "Infinity"
            : userPlanLimits.deliverabilityChecks,
        personalizedEmails:
          userPlanLimits.personalizedEmails === Infinity
            ? "Infinity"
            : userPlanLimits.personalizedEmails,
        campaigns:
          userPlanLimits.campaigns === Infinity
            ? "Infinity"
            : userPlanLimits.campaigns,
        warmupEmails:
          userPlanLimits.warmupEmails === Infinity
            ? "Infinity"
            : userPlanLimits.warmupEmails,
        emailAccounts:
          userPlanLimits.emailAccounts === Infinity
            ? "Infinity"
            : userPlanLimits.emailAccounts,
        dailyLimit:
          userPlanLimits.dailyLimit === Infinity
            ? "Infinity"
            : userPlanLimits.dailyLimit,
      };
  ...
  app.post(
    "/api/email-integrations/gmail-auth-url",
    isAuthenticated,
    async (req: any, res) => {
      try {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!canAdd) {
          return res
            .status(403)
            .json({ message: "Plan limit reached for email integrations" });
        }
        ...
        if (data.connectionType === "smtp") {
          ...
          if (!isValid) {
            return res
              .status(400)
              .json({ message: "Email integration validation failed" });
          }
        }
        ...
        if (data.connectionType === "smtp") {
          await storage.updateEmailIntegrationVerification(
            integration.id,
            true
          );
        }
  ...
  app.post(
    "/api/email-integrations/:id/toggle-warmup",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.get(
    "/api/recipient-lists/:id/personalization-status",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        const totalRecipients = recipients.length;
        const personalizedRecipients = recipients.filter(
          (r) => r.personalizedEmail && r.personalizedEmail.trim() !== ""
        ).length;
        const hasAllPersonalized =
          totalRecipients > 0 && personalizedRecipients === totalRecipients;
        ...
        const sampleData = recipients.slice(0, 3).map((recipient) => ({
          ...
          personalizedEmailPreview: recipient.personalizedEmail
            ? recipient.personalizedEmail.substring(0, 150) + "..."
            : null,
        }));
        ...
        res.json({
          ...
          personalizationPercentage:
            totalRecipients > 0
              ? Math.round((personalizedRecipients / totalRecipients) * 100)
              : 0,
          sampleData,
        });
      } catch (error) {
        ...
        res
          .status(500)
          .json({ message: "Failed to get personalization status" });
      }
    }
  );
  ...
  app.get(
    "/api/recipient-lists/:id/recipients",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const listId = parseInt(req.params.id);
      ...
      try {
        ...
        if (!canAdd) {
          return res.status(403).json({
            message:
              "Your uploaded list exceeds your plan limit. Please upgrade your plan to add more recipients.",
          });
        }
        ...
        await storage.updateUsage(userId, currentMonth, {
          recipientsUploaded:
            (usage?.recipientsUploaded || 0) + recipients.length,
        });
  ...
  app.post(
    "/api/recipients/:id/check-deliverability",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        if (!canCheck) {
          return res
            .status(403)
            .json({ message: "Plan limit reached for deliverability checks" });
        }
        ...
        try {
          const validationResult = await emailValidationService.validateEmail(
            recipient.email
          );
          const status =
            emailValidationService.getDeliverabilityStatus(validationResult);
          ...
          res.json({
            ...
            reason:
              emailValidationService.getValidationReason(validationResult),
          });
        } catch (error) {
          console.error("Email validation error:", error);
          // Fallback to basic validation
          const fallbackStatus = "risky"; // Conservative fallback
          await storage.updateRecipientDeliverability(
            recipientId,
            fallbackStatus
          );
  ...
  app.post(
    "/api/recipient-lists/:id/check-deliverability",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        // Check plan limits
        const canCheck = await checkPlanLimits(
          userId,
          "deliverability",
          recipients.length
        );
        ...
        try {
          const validationResults = await emailValidationService.validateEmails(
            emails
          );
        ...
        await storage.updateUsage(userId, currentMonth, {
          deliverabilityChecks:
            (usage?.deliverabilityChecks || 0) + recipients.length,
        });
  ...
  app.post(
    "/api/recipient-lists/:id/remove-invalid",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        res
          .status(500)
          .json({ message: "Failed to remove invalid recipients" });
      }
    }
  );
  ...
  app.get(
    "/api/recipient-lists/:id/export-clean",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="clean-list-${Date.now()}.csv"`
        );
        res.send(csvContent);
      } catch (error) {
  ...
  app.get(
    "/api/recipient-lists/:id/validation-stats",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const listId = parseInt(req.params.id);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        console.log("Personalizing recipient:", {
          recipientId,
          userId,
          emailType,
          tone,
        ...
        if (!canPersonalize) {
          return res
            .status(403)
            .json({ message: "Plan limit reached for email personalization" });
        }
        ...
        console.log("Found recipient with website:", {
          id: recipient.id,
          website: recipient.websiteLink,
        });
        ...
        if (recipient.websiteLink) {
          console.log(
            "Scraping website for personalization:",
            recipient.websiteLink
          );
          websiteContent = await scrapeWebsiteContent(recipient.websiteLink);
          if (websiteContent) {
            console.log(
              "Successfully scraped website content, length:",
              websiteContent.length
            );
          } else {
            console.log(
              "Failed to scrape website content, proceeding without it"
            );
          }
        }

        const personalizedEmail = await personalizeEmailContent(
          recipient,
          websiteContent,
          {
            emailType,
        ...
        console.log(
          "Generated personalized email, length:",
          personalizedEmail.length
        );
        console.log(
          "Full personalized email content:",
          personalizedEmail.substring(0, 200) + "..."
        );

        await storage.updateRecipientPersonalizedEmail(
          recipientId,
          personalizedEmail
        );
        ...
        res
          .status(500)
          .json({ message: `Failed to personalize email: ${error.message}` });
      }
    }
  );

  app.post(
    "/api/recipient-lists/:id/personalize",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        const recipients = await storage.getListRecipients(listId);
        const unPersonalizedRecipients = recipients.filter(
          (r) => r.websiteLink && !r.personalizedEmail
        );

        // Check plan limits
        const canPersonalize = await checkPlanLimits(
          userId,
          "personalization",
          unPersonalizedRecipients.length
        );
        if (!canPersonalize) {
          return res
            .status(403)
        ...
        for (const recipient of unPersonalizedRecipients) {
          try {
            ...
            if (recipient.websiteLink) {
              console.log(
                `Scraping website for recipient ${recipient.id}:`,
                recipient.websiteLink
              );
              websiteContent = await scrapeWebsiteContent(
                recipient.websiteLink
              );
              if (websiteContent) {
                console.log(
                  `Successfully scraped website content for ${recipient.id}, length:`,
                  websiteContent.length
                );
              }
            }
            ...
            console.error(
              `Error personalizing email for recipient ${recipient.id}:`,
              error
            );
        ...
        try {
          ...
          await storage.updateUsage(userId, currentMonth, {
            personalizedEmails:
              (usage?.personalizedEmails || 0) + personalizedCount,
          });
        } catch (usageError) {
          console.error(
            "Error updating bulk usage (non-critical):",
            usageError
          );
        }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const csvRows = [
          ...
          ...recipients.map((recipient) =>
            [
              ...
              `"${
                recipient.personalizedEmail ||
                (recipient.websiteLink ? "Not generated" : "No website")
              }"`,
            ].join(",")
          ),
        ];
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=personalized-emails-${listId}.csv`
        );
        res.send(csvContent);
      } catch (error) {
  ...
  app.post(
    "/api/recipient-lists/:targetListId/update-with-personalized",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId =
        ...
        const personalizedRecipients = sourceRecipients.filter(
          (r) => r.personalizedEmail
        );

        if (personalizedRecipients.length === 0) {
          return res
            .status(400)
            .json({ message: "No personalized emails found in source list" });
        }
        ...
        const existingRecipients = await storage.getListRecipients(
          targetListId
        );
        ...
        for (const personalizedRecipient of personalizedRecipients) {
          ...
          const existingRecipient = existingRecipients.find(
            (r) => r.email === personalizedRecipient.email
          );
          ...
          if (existingRecipient) {
            ...
            await storage.updateRecipientPersonalizedEmail(
              ...
              personalizedRecipient.personalizedEmail
            );
            updatedCount++;
          } else {
  ...
  app.post("/api/campaigns", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "Missing required fields: name, recipientListId, emailIntegrationId, subject, body",
        });
      }
      ...
      if (existingCampaigns.length >= limits.campaigns) {
        return res
          .status(403)
          .json({ message: "Plan limit reached for campaigns" });
      }
  ...
  app.patch(
    ...
    async (req: any, res) => {
      try {
        ...
        if (
          !status ||
          !["draft", "scheduled", "sending", "paused", "completed"].includes(
            status
          )
        ) {
        ...
        const updatedCampaign = await storage.updateCampaignStatus(
          campaignId,
          status
        );
  ...
  app.patch("/api/campaigns/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (campaign.status !== "draft" && campaign.status !== "paused") {
        return res
          .status(400)
          .json({ message: "Campaign cannot be edited while active" });
      }

      const updateData: any = {};
      if (subject !== undefined) updateData.subject = subject;
      if (body !== undefined) updateData.body = body;
      if (followUpSubject !== undefined)
        updateData.followUpSubject = followUpSubject;
      if (followUpBody !== undefined) updateData.followUpBody = followUpBody;

      const updatedCampaign = await storage.updateCampaign(
        campaignId,
        updateData
      );
      res.json(updatedCampaign);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const limits =
          planLimits[user?.plan as keyof typeof planLimits] || planLimits.demo;
        const existingFollowUps = await storage.getCampaignFollowUps(
          campaignId
        );
        ...
        if (existingFollowUps.length >= limits.followUps) {
          return res
            .status(403)
            .json({ message: "Plan limit reached for follow-ups" });
        }
  ...
  app.get("/api/warmup/stats", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const statsPromises = integrations.map(
        async (integration: EmailIntegration) => {
          const fullStats = await storage.getWarmupStatsForIntegration(
            integration.id
          );
          const warmupScore = await storage.calculateWarmupScore(
            integration.id
          );
  ...
  app.post(
    "/api/warmup/initialize/:integrationId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        const userIntegration = integration.find(
          (i) => i.id === parseInt(integrationId)
        );
  ...
  app.post("/api/warmup/send", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const integrations = await storage.getUserEmailIntegrations(userId);
      const warmupIntegrations = integrations.filter(
        (i) => i.warmupEnabled && i.isVerified
      );

      console.log(
        `Found ${integrations.length} total integrations, ${warmupIntegrations.length} warmup-enabled and verified`
      );
      console.log(
        "Integrations:",
        integrations.map((i) => ({
      ...
      if (warmupIntegrations.length === 0) {
        return res.status(400).json({
          message: "No verified email integrations enabled for warmup",
        });
      }
      ...
      for (const integration of warmupIntegrations) {
        ...
        console.log(
          `Initialized and reset warmup progress for integration ${integration.id} (${integration.email})`
        );
      }
      ...
      console.log(
        `Calling warmupService.sendWarmupEmails for user ${userId} with ${warmupIntegrations.length} integrations`
      );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        if (warmupEmail) {
          await warmupService.updateWarmupStats(warmupEmail.fromIntegrationId, {
            emailsOpened: 1,
          });
        }
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        await warmupService.processWarmupEmailAction(
          parseInt(emailId),
          action,
          replyBody
        );
  ...
  app.get("/api/track/click/:trackingId", async (req, res) => {
    try {
      ...
      console.log(
        `Email link clicked - tracking ID: ${trackingId}, URL: ${url}`
      );
  ...
  app.post("/api/warmup/debug-reset", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({
        message: "Warmup progress reset and reinitialized successfully",
      });
    } catch (error) {
